...
ViivanPiste::ViivanPiste(float x_, float y_, float paine_, ofVec2f nopeus_) : x(x_), y(y_), paine(paine_) {
}
...
void Viiva::lisaaPiste(float x, float y, float paine) {
    ...
    ofVec2f uusiNopeus = uusiSijainti - viimeSijainti;
    ofVec2f viimeNopeus = viimeSijainti - toissaSijainti;
    ...
    std::vector<float> paksuudet = haeArvot(&paksuus, OTANNAN_KOKO);
